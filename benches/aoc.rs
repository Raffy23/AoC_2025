...
fn day08_benchmark(c: &mut Criterion) {
    let input = read_input(8, None).expect("Unable to read input file!");

    c.bench_function("Day08 Part1", |b| {
        b.iter(|| day08::solve1(black_box(&mut input.as_str())))
    });

    c.bench_function("Day08 Part2", |b| {
        b.iter(|| day08::solve2(black_box(&mut input.as_str())))
    });
}

fn day09_benchmark(c: &mut Criterion) {
    let input = read_input(9, None).expect("Unable to read input file!");

    c.bench_function("Day09 Part1", |b| {
        b.iter(|| day09::solve1(black_box(&mut input.as_str())))
    });

    c.bench_function("Day09 Part2", |b| {
        b.iter(|| day09::solve2(black_box(&mut input.as_str())))
    });
}

fn day10_benchmark(c: &mut Criterion) {
    let input = read_input(10, None).expect("Unable to read input file!");

    c.bench_function("Day10 Part1", |b| {
        b.iter(|| day10::solve1(black_box(&mut input.as_str())))
    });
...
fn day12_benchmark(c: &mut Criterion) {
    let input = read_input(12, None).expect("Unable to read input file!");

    c.bench_function("Day12 Part1", |b| {
        b.iter(|| day12::solve1(black_box(&mut input.as_str())))
    });
}
